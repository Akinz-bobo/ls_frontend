...
export class UnifiedAudioSystem {
  ...
  async addAudioSource(
    ...
    try {
      ...
      this.updateMetrics();
      console.log(
        `🎤 Added audio source: ${sourceConfig.name} (${sourceConfig.type})`
      );
    } catch (error) {
  ...
  async startBroadcast(): Promise<void> {
    try {
      ...
      this.isActive = true;
      console.log(
        "✅ Broadcast started successfully with",
  ...
  private async startAudioStreaming(): Promise<void> {
    if (!this.destinationStream || !this.socket?.connected) {
      console.warn(
        "🎵 Cannot start audio streaming: missing destination stream or socket connection"
      );
      return;
    }
    ...
    try {
      ...
      // Last resort: WebM (but warn about streaming chunks)
        ...
        console.warn(
          "⚠️ Using WebM format - may create streaming chunks instead of complete files"
        );
      } else {
...
export class UnifiedAudioListener {
  ...
  async startListening(): Promise<void> {
    try {
      console.log(
        "🎧 Starting WebRTC listener for broadcast:",
        this.broadcastId
      );
        ...
        await new Promise<void>((resolve, reject) => {
          this.mediaSource!.addEventListener("sourceopen", () => {
            ...
            try {
              ...
              console.log(
                `🎵 Creating SourceBuffer with MIME type: ${mimeType}`
              );
  ...
  private async connectToServer(): Promise<void> {
    return new Promise<void>((resolve, reject) => {
      ...
      this.socket.on("connect", () => {
        ...
        console.log(
          `🏠 Emitting join-broadcast for broadcastId: ${this.broadcastId}`
        );
  ...
  private async handleAudioChunk(base64Audio: string): Promise<void> {
    console.log(
      "🎵 handleAudioChunk called with audio length:",
      base64Audio.length
    );
    ...
    try {
      ...
      if (this.useSimpleBlobApproach) {
        try {
          ...
          console.warn(
            "⚠️ Blob approach failed, falling back to MediaSource:",
      ...
      console.log(
        "🎵 Using MediaSource for streaming audio chunks (skipping Web Audio API)"
      );
        ...
        try {
          ...
          if (this.mediaSource.readyState === "closed") {
            ...
            if (
              ...
              console.warn(
                "⚠️ Failed to recreate MediaSource, dropping audio chunk"
              );
              return;
            }
          }
          ...
          console.log(
            `📦 Chunk validation: ${isValidChunk ? "✅ Valid audio" : "❌ Invalid audio"} chunk`
          );
          ...
          if (isValidChunk) {
            ...
            if (boxType === "ftyp" && !this.hasInitializationSegment) {
              console.log(
                `🎬 Found complete MP4 file with initialization (${bytes.length} bytes)`
              );
            ...
            // Also handle separate moov boxes if they come later
              ...
              console.log(
                `🎬 Found separate initialization segment: ${boxType}`
              );
              this.initializationSegment = bytes;
              this.hasInitializationSegment = true;
              ...
              if (this.sourceBuffer && !this.sourceBuffer.updating) {
                try {
                  this.sourceBuffer.appendBuffer(bytes);
                  console.log(
                    "✅ Separate initialization segment added to MediaSource"
                  );
                } catch (error) {
                  console.error(
                    "❌ Failed to add initialization segment:",
                    error
                  );
                }
              }
            ...
            if (!this.hasInitializationSegment && boxType === "moof") {
              console.warn(
                "⚠️ Received media fragment without initialization - waiting for init segment"
              );
              return;
            }
            ...
            if (
              ...
              console.log(
                `📦 Streaming: ${this.pendingAudioChunks.length} chunks queued (init: ${this.hasInitializationSegment})`
              );
            }
  ...
  private processPendingChunks(): void {
    ...
    if (this.mediaSource.readyState !== "open") {
      console.warn(
        "⚠️ MediaSource not in open state:",
    ...
    try {
      ...
      if (this.pendingAudioChunks.length === 0 || Math.random() < 0.2) {
        console.log(
          `🎵 Processing audio chunk: ${chunk.length} bytes (${this.pendingAudioChunks.length} in queue)`
        );
      }
      ...
      if (
        ...
        this.audioElement
          ...
          .catch((error) => {
            console.warn(
              "⚠️ Could not start playback (will retry):",
              error instanceof Error ? error.message : String(error)
            );
          });
      }
  ...
  private clearOldBufferData(): void {
    ...
    try {
      ...
      if (this.sourceBuffer.buffered.length > 0 && clearBefore > 0) {
        console.log(
          `💾 Clearing buffer data before ${clearBefore.toFixed(2)}s`
        );
      ...
      await new Promise<void>((resolve, reject) => {
        ...
        this.mediaSource!.addEventListener("sourceopen", () => {
          ...
          try {
            ...
            console.log(
              `🔄 Creating new SourceBuffer with MIME type: ${mimeType}`
            );
            this.sourceBuffer = this.mediaSource!.addSourceBuffer(mimeType);
            this.sourceBuffer.addEventListener("updateend", () => {
              this.processPendingChunks();
    ...
    if (this.lastPlayTime === 0 || now - this.lastPlayTime > 500) {
      if (this.audioChunkBuffer.length > 0) {
        try {
          ...
          console.log(
            `🎵 Played combined audio chunk: ${combinedSize} bytes from ${this.audioChunkBuffer.length} chunks`
          );
        } catch (error) {
          console.warn(
            "⚠️ Combined chunk playback failed:",
            error instanceof Error ? error.message : String(error)
          );
  ...
  private async playAudioWithBlob(bytes: Uint8Array): Promise<void> {
    ...
    try {
      ...
      console.log(
        `🎵 ${mimeType} audio chunk playing successfully (2s duration)`
      );
