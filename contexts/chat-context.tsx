...
type ChatAction =
  ...
  | {
      type: "SET_BROADCAST_INFO";
      payload: { id: string; title: string; startTime: Date };
    }
  ...
  | {
      ...
      payload: Partial<ChatState["chatSettings"]>;
    }
  | { type: "CLEAR_MESSAGES" };
    ...
    case "REMOVE_USER":
      return {
        ...
        typingUsers: state.typingUsers.filter(
          (typing) => typing.userId !== action.payload
        ),
      };
...
export function ChatProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    socket.on("new-message", (data: any) => {
      ...
      const message: ChatMessage = {
        ...
        isHighlighted:
          data.isHighlighted || data.messageType === "announcement",
    ...
    socket.on("user-banned", (data: any) => {
      toast.error(
        `You have been banned from this chat. Reason: ${data.reason || "No reason provided"}`
      );
      ...
      if (response.success && response.data) {
        ...
        messages?.forEach((msg: any) => {
          const message: ChatMessage = {
            id: msg.id,
            broadcastId:
              response.data.broadcastId || state.currentBroadcast || "",
            userId: msg.userId,
            ...
            dislikes: 0,
            isLiked: msg.likedBy
              ? msg.likedBy.includes(state.currentUser?.id)
              : false,
            isDisliked: false,
    ...
    socket.on("users-updated", (data: any) => {
      ...
      data.users?.forEach((user: any) => {
        ...
        dispatch({
          type: "UPDATE_USER",
          payload: { id: user.id, updates: chatUser },
        });
      });
    });
    ...
    socket.on("message-moderated", (data: any) => {
      ...
      if (data.action === "delete") {
        ...
        if (actionMessages[data.action]) {
          toast.info(
            `Message ${actionMessages[data.action]} by ${data.moderatedBy}`
          );
        }
      }
    });
    ...
    socket.on("broadcast-status-success", (response: any) => {
      if (response.success) {
        console.log(
          "✅ Broadcast status updated successfully:",
          response.message
        );
      }
    });
  ...
  const sendMessage = async (
    ...
    if (content.length > state.chatSettings.maxMessageLength) {
      toast.error(
        `Message too long. Max ${state.chatSettings.maxMessageLength} characters.`
      );
      return;
    }
    ...
    if (messageType === "announcement") {
      socketRef.current.emit("send-announcement", state.currentBroadcast, {
        content: content.trim(),
        username: state.currentUser.username,
        isStaff: ["host", "moderator", "admin"].includes(
  ...
  const joinBroadcast = async (broadcastId: string, user: ChatUser) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log(
          `📚 Loaded ${messages.length} chat messages for broadcast ${broadcastId}`
        );
        // Add historical messages to state
        messages.forEach((msg: any) => {
          const message: ChatMessage = {
  ...
  const setBroadcastLive = (
    ...
    console.log(
      `📻 Broadcast status changed: ${isLive ? "LIVE" : "OFFLINE"}`,
      broadcastInfo
    );
  };
