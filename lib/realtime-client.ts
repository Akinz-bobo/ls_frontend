...
export class RealtimeClient {
  ...
  sendMessage(
    broadcastId: string,
    message: { content: string; messageType?: string; replyTo?: string }
  ) {
  ...
  joinAsBroadcaster(
    broadcastId: string,
    broadcasterInfo?: { username?: string; stationName?: string }
  ) {
  ...
  broadcastAudio(
    broadcastId: string,
    audioData: { audio: string; timestamp: number; metrics?: any }
  ) {
  ...
  requestCall(
    broadcastId: string,
    callerInfo: { name: string; location?: string }
  ) {
  ...
  onAudioSourceRemoved(
    callback: (data: { broadcastId: string; sourceId: string }) => void
  ) {
  ...
  onCallPending(
    callback: (data: { callId: string; position: number }) => void
  ) {
  ...
  onCallQueueUpdate(
    callback: (data: { queue: CallRequest[]; activeCalls: any[] }) => void
  ) {
  ...
  onBroadcastEnded(
    callback: (data: { reason: string; stats: any; endTime: string }) => void
  ) {
